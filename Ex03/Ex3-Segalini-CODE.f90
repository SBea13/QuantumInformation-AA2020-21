...
!   debugging process: check all matrix sizes and if the product is legit
    if (debug) then
        call debugging(rowa > 0, "#Rows(A) > 0", verbose, stopprg, rowa)
        call debugging(cola > 0, "#Cols(A) > 0", verbose, stopprg, cola)
        call debugging(rowb > 0, "#Rows(B) > 0", verbose, stopprg, rowb)
        call debugging(colb > 0, "#Cols(A) > 0", verbose, stopprg, colb)
        call debugging(cola == rowb, "#Rows(B) = #Cols(A)", verbose, stopprg, cola)
    end if
